...
export class EcoMetricsClient implements PkCore.IEcoMetricsClient {
  ...
  public async getMemoryMetricsFromInfrastructure(): Promise<AxiosResponse> {
    const client = await this.dynamicClient.getClient("eco-os-metrics-service");
    return await client.getClient().get("/metrics/memory");
  }
}
